...
class DjangoAPIMixin:
    """Our custom Django API base class mixin.* 
    
    Created to standardize handling request and response on all modules.
    ...
    RESPONSE class atrribute must be set on child classes as class attributes as well.
    The values of which is either one of the valid RESPONSE_CLASSES listed:
    - rest_framewor's Response 
               or
    - django's JsonResponse


    * This is written here instead of under utils.mixins, 
    for convenience of implementation users, readers and reviewers.
    
    Author: 
        Edchel Stephen Nini(ESN)
        yahshua.ninies@gmail.com, edchelstephens@gmail.com
    """

    status = 200

    # NOTE: Override this on child classes
    RESPONSE = None 
    ...
    # If called from another view or caller such as cron tasks
    # NOTE: Set this on your views
    # def post(self, request, record=None, fromAPI=False):
    # try:
    #   self.IS_API_CALL = bool(record) or fromAPI
    IS_API_CALL = False
    ...
    error_dict = {
        ...
        "description":  "Something went wrong. Please contact system administrator.", # description key is used in Desktop
        "errors": None # Put errors here e.g. serializer.errors, etc
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            elif self.RESPONSE not in self.RESPONSE_CLASSES:
                raise TypeError("Invalid RESPONSE object, must be one of {}".format(self.RESPONSE_CLASSES))
    ...
    def get_response(self, data, status, content_type):
        ...
        return data if self.IS_API_CALL else self.RESPONSE(data=data, status=status, content_type=content_type)
    ...
    def early_success_response(self):
        """Method for returning early exit success response."""
        data = { "status": "OK" }
        return self.success_response(data)

    def basic_error_response(self, error_data, status=400):
        """Method to return simple rest framework error Response."""
        if not self.is_valid_error_code(status):
            status=400
        return self.RESPONSE(data=error_data, status=status)
